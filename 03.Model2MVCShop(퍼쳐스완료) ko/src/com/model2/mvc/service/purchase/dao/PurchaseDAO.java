...
public class PurchaseDAO {
	...
	public Purchase findPurchase(int tranNo) throws Exception{
		...
		while(rs.next()){
			...
			purchase.setTranNo(rs.getInt("tran_no")); //구매번호
			purchase.setPaymentOption(rs.getString("payment_option"));//지불방식
			purchase.setReceiverName(rs.getString("receiver_name"));//받는 사람 이름
			purchase.setReceiverPhone(rs.getString("receiver_phone"));//받는 사람 전화번호
			purchase.setDivyAddr(rs.getString("demailaddr"));//배송지 주소
			purchase.setDivyRequest(rs.getString("dlvy_request"));//배송시 요구사항
			purchase.setTranCode(rs.getString("tran_status_code"));//구매상태 코드
			purchase.setOrderDate(rs.getDate("order_data"));//java.sql.date타입의 구매일자
			purchase.setDivyDate(rs.getString("dlvy_date"));//배송희망일자
			...
			purchase.setBuyer(user);//Buyer-UserVO타입의 구매자 정보
			purchase.setPurchaseProd(product);//PurchaseProd-ProductVO타입의 구매물품정보
